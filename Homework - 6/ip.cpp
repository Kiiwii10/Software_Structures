...
{   
    ...
    packet.split(",", NULL, &split_size);
    if ((split_size != 1) && (split_size != 4))
    {
        return false;
    }
    String* initial_split = NULL;
...
bool ip_split_type(String& packet_field, String& type_output, String& str_ip){
    size_t size_split = 0;
    (packet_field).split("=", NULL, &size_split);
    if (size_split != 2)
    {
        return false;
    }
    String* packet_split = NULL;
